...
11. JS原型是什么？原型链是什么？
    ...
    一个对象 通过原型 获得了一个函数或者属性，每个对象都有一个 __proto__ 属性，并且指向它的 prototype 原型对象。
   ...
   每个对象都有一个 __proto__ 属性，并且指向它的 prototype 原型对象，而 prototype 原型又具有自己的 prototype 原型对象，就这样一直往上知道一个对象的原型 prototype 为 null。
...
14. 函数节流和函数防抖
    ...
    ```javascript
     ...
     ```
 15. 
     - map(入参是函数，返回一个新数组，数组中的元素是被函数处理后的值)
     - reduce(入参是函数，数组中每个元素执行完函数后将结果汇总为单个返回值)
     - filter(入参是函数，返回一个新数组，数组中的元素通过函数的测试)
     - splice(第一个入参是整数规定位置，使用负数则从结尾开始，第二个是要删除的项目的数量，如果是0就不会删，第三个可选，是像数组里面添加项目)
     - split(把一个**字符串**分割成自字符串数组)
     - new 构造函数 跟原型跟原型链有关。
...
2. 什么是 CSRF 攻击？ 如何预防？
    ...
    预防方法有在请求地址添加 token 并验证，服务端收到路由请求时生成一个随机数在渲染请求页面时将随机数埋入页面（一般埋入 form 表单），服务端设置 setcookies，把该随机数作为 cookies 或者 session 种如用户浏览器，当用户发送请求时带上 token 的参数，后台进行对比，如果相等就是合法的。
